...
import {
  ...
  LogOut, ChevronRight, RefreshCw, XCircle, Eye,
} from 'lucide-react';
...
const statusConfig: Record<string, { label: string; icon: any; color: string; bg: string }> = {
  pending_payment: { label: 'Ожидает оплаты', icon: Clock, color: 'text-yellow-700', bg: 'bg-yellow-100' },
  preparing: { label: 'Готовится', icon: Package, color: 'text-orange-700', bg: 'bg-orange-100' },
  on_the_way: { label: 'В пути', icon: Truck, color: 'text-blue-700', bg: 'bg-blue-100' },
  delivered: { label: 'Доставлен', icon: CheckCircle, color: 'text-green-700', bg: 'bg-green-100' },
    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-orange-50 to-amber-50 py-6 px-4">
      <div className="container mx-auto max-w-5xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold text-orange-600">Личный кабинет</h1>
          <Button
            variant="outline"
            ...
            <Card className="shadow-lg border-2 border-orange-200">
              <CardHeader className="bg-gradient-to-r from-orange-400 to-yellow-400 rounded-t-lg pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-bold text-lg">{user.name || 'Пользователь'}</p>
                ...
                {!editMode ? (
                  <>
                    ...
                    <div className="flex items-center gap-2 text-gray-700">
                      ...
                      <span className="text-sm">Заказов: {orders?.length || 0}</span>
                    </div>
                    <Button
              ...
              <CardContent className="pt-4 space-y-2">
                <Link href="/menu">
                  <div className="flex items-center justify-between p-3 rounded-lg hover:bg-orange-50 cursor-pointer transition-colors">
                    <span className="font-medium text-gray-700">Меню</span>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="shadow-lg border-2 border-orange-200">
              <CardHeader className="bg-gradient-to-r from-orange-400 to-yellow-400 rounded-t-lg">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-white text-lg">Текущие заказы</CardTitle>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => refetch()}
                      ...
                      return (
                        <div key={order.id} className="p-4 bg-white border-2 border-orange-100 rounded-xl shadow-sm">
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              <p className="font-bold text-gray-800">Заказ #{order.orderNumber}</p>
                          ...
                          <div className="flex justify-between items-center">
                            <div className="text-sm text-gray-600">
                              <p>{order.items.length} позиц. • {Number(order.totalAmount).toFixed(0)} ₽</p>
                              {order.estimatedTime && (
                                <p className="text-orange-600 font-medium">⏱ ~{order.estimatedTime} мин</p>
                              )}
                            </div>
            ...
            <Card className="shadow-lg border-2 border-gray-200">
              <CardHeader className="bg-gradient-to-r from-gray-500 to-gray-600 rounded-t-lg">
                <CardTitle className="text-white text-lg">История заказов</CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
                {isLoading ? (
                  <div className="flex justify-center py-8"><LoadingSpinner /></div>
                ) : pastOrders.length === 0 ? (
                  <p className="text-center text-gray-500 py-6">История заказов пуста</p>
                ) : (
                        ...
                        <div key={order.id} className="p-3 bg-gray-50 border border-gray-200 rounded-xl flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              {new Date(order.createdAt).toLocaleDateString('ru-RU')} • {order.items.length} позиц. • {Number(order.totalAmount).toFixed(0)} ₽
                            </p>
                          </div>
