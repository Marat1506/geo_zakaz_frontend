                    ...
                    <div className="flex flex-wrap items-start gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-3 text-sm text-gray-500">
                          <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" />{new Date(order.createdAt).toLocaleString()}</span>
                          <span className="flex items-center gap-1"><Car className="h-3.5 w-3.5" />{order.carPlateNumber} · {order.carColor}</span>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {s.next && (
                          <Button size="sm" className={`text-white min-h-[40px] text-xs ${s.nextColor}`} onClick={() => handleStatusUpdate(order.id, s.next!)} disabled={isUpdating}>
                            {isUpdating ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : s.nextLabel}
                          </Button>
                        )}
                        {order.status !== 'delivered' && order.status !== 'cancelled' && (
                          <Button size="sm" variant="outline" className="border-red-300 text-red-500 hover:bg-red-50 min-h-[40px] text-xs" onClick={() => handleStatusUpdate(order.id, 'cancelled')} disabled={isUpdating}>
                            Cancel
                          </Button>
                        )}
                        <Button size="sm" variant="ghost" className="min-h-[40px] min-w-[40px] p-0" onClick={() => setExpandedId(isExpanded ? null : order.id)}>
